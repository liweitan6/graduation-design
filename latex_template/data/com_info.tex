% !Mode:: "TeX:UTF-8"

% 学院中英文名，中文不需要“学院”二字
% 院系英文名可从以下导航页面进入各个学院的主页查看
% https://www.buaa.edu.cn/jgsz/yxsz.htm
\school
{计算机}{}

% 专业中英文名
\major
{计算机科学与技术}{}

% 论文中英文标题
\thesistitle
{面向深度学习系统模糊测试的测试用例智能管理与分析}
{}
{Intelligent Management and Analysis of Test Cases for Deep Learning System Fuzzing}
{}

% 作者中英文名
\thesisauthor
{郭俊杰}{Guo Junjie}

% 导师中英文名
\teacher
{吴际}{Wu Ji}
% 副导师中英文名
% 注：慎用‘副导师’，见北航研究生毕业论文规范
%\subteacher{(副导师姓名)}{(Name of Subteacher)}

% 中图分类号，可在 http://www.ztflh.com/ 查询
\category{TP311.5}

% 本科生为毕设开始时间；研究生为学习开始时间
\thesisbegin{2025}{12}{15}

% 本科生为毕设结束时间；研究生为学习结束时间
\thesisend{2026}{5}{31}

% 毕设答辩时间
\defense{2026}{6}{10}

% 中文摘要关键字
\ckeyword{深度学习框架，模糊测试，语义检索，检索增强生成，覆盖效能指数}

% 英文摘要关键字
\ekeyword{Deep Learning Framework, Fuzz Testing, Semantic Retrieval, Retrieval-Augmented Generation, Coverage Efficiency Index}
